...
from telethon.tl.functions.phone import (
    RequestCallRequest, DiscardCallRequest,
)
...
async def make_single_call(
    client, target_user, ring_seconds, name
) -> bool:
    try:
        dh_config = await client(
        ...
        call_id = call.id
        call_peer = InputPhoneCall(
            id=call.id, access_hash=call.access_hash
        )

        print_call(
            f"{name} — звонок идёт (макс. {ring_seconds} сек)..."
        )
        ...
        try:
            ...
            while elapsed < ring_seconds:
                ...
                if call_accepted.is_set():
                    print_call(
                        f"{name} — поднял трубку → сброс!"
                    )
                    break
                if call_ended.is_set():
                    print_dim(
                        f"{name} — отклонён собеседником."
                    )
                    return True
            else:
                print_dim(
                    f"{name} — таймаут {ring_seconds}с → сброс."
                )
        ...
        print_error(
            f"{name} — ошибка звонка: {type(e).__name__}: {e}"
        )
    return False


async def do_call_session(
    session_path, target_info, ring_seconds,
    ...
    try:
        ...
        ok = await make_single_call(
            client, entity, ring_seconds, name
        )
...
async def do_combo_session(
    ...
    try:
        ...
        if combo_order == "call_first":
            ok_call = await make_single_call(
                client, entity, ring_seconds, name
            )
    ...
    try:
        while True:
            ...
            for i, sess in enumerate(sessions):
                if action == "1":
                    ok = await do_call_session(
                        sess, target_info, ring_seconds,
                        delete_after, delete_delay,
                    )
                else:
                    msg = (
